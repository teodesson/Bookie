...
class AdminApiTest(unittest.TestCase):
    ...
    def test_invite_ct(self):
        ...
        data = json.loads(res.body.decode('utf-8'))['users']
        found = False
        invite_count = None
        ...
        self.assertTrue(found, "There should be the admin user." + str(res.body))
        self.assertEqual(
            0,
            invite_count,
            "The admin user shouldn't have any invites." + res.body.decode('utf-8'))
    ...
    def test_set_invite_ct(self):
        ...
        self.assertEqual(
            'admin',
            data.get('username'),
            "The admin user data is returned to us." + str(res.body))
        self.assertEqual(
            10,
            int(data.get('invite_ct')),
            "The admin user now has 10 invites." + res.body.decode('utf-8'))
    ...
    def test_import_info(self):
        ...
        self.assertEqual(
            1, data.get('count'), "There are none by default. " + res.body.decode('utf-8'))

        self.assertEqual(
            'admin',
            data.get('imports')[0]['username'],
            "The first import is from admin " + res.body.decode('utf-8'))
        self.assertEqual(
            0,
            data.get('imports')[0]['status'],
            "And it has a status of 0 " + res.body.decode('utf-8'))
    ...
    def test_user_list(self):
        ...
        self.assertEqual(
            ...
            "The first user is from admin " + res.body.decode('utf-8'))
        self.assertEqual(
            'testing@dummy.com',
            data.get('users')[0]['email'],
            "The first user is from testing@dummy.com " + res.body.decode('utf-8'))
